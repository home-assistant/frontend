...
export class HaServiceAction extends LitElement implements ActionElement {
    ...
    return html`
      ...
      ${domain && service && this.hass.services[domain]?.[service]?.response
        ? html`<ha-settings-row
            .narrow=${this.narrow}
            class=${this.hass.services[domain][service].response!.optional
              ? "clickable"
              : ""}
            @click=${this.hass.services[domain][service].response!.optional
              ? this._responseRowClicked
              : nothing}
          >
            <span slot="heading"
              >${this.hass.localize(
            ...
            ${this.hass.services[domain][service].response!.optional
              ? html`<ha-switch
                  ...
                  .disabled=${this.disabled}
                  @change=${this._responseSwitchChanged}
  ...
  private _responseRowClicked(ev: Event) {
    if (this.disabled) {
      return;
    }
    const target = ev.target as HTMLElement;
    if (target.closest("ha-textfield")) {
      return;
    }
    const isEnabled =
      !!this._action?.response_variable || this._responseChecked;
    this._responseSwitchChanged({
      target: { checked: !isEnabled },
    } as any);
  }

  private _responseSwitchChanged(ev) {
