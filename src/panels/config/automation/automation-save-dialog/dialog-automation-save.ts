...
import { fetchLLMTaskPreferences, runLLMTask } from "../../../../data/llm_task";
...
class DialogAutomationSave extends LitElement implements HassDialog {
  ...
  @state() private _summaryEntity: string | undefined;
  ...
  protected firstUpdated(changedProperties: PropertyValues): void {
    ...
    if (isComponentLoaded(this.hass, "llm_task")) {
      fetchLLMTaskPreferences(this.hass).then((prefs) => {
        if (prefs.summary_entity_id) {
          this._summaryEntity = prefs.summary_entity_id;
        }
      });
    }
  }
  ...
  private async _suggest() {
    const result = await runLLMTask(
      this.hass,
      "frontend:automation:save",
      this._summaryEntity!,
      "summary",
      `Give me a name for the following Home Assistant automation.
