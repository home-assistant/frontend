...
import { manualEditorStyles } from "../../automation/styles";
...
export class HaBlueprintEditor extends PreventUnsavedMixin(
    ...
    try {
      ...
      showToast(this, {
        message: this.hass.localize(
          "ui.panel.config.developer-tools.tabs.blueprints.paste_invalid_config"
        ),
        duration: 4000,
        dismissable: true,
      });
  ...
  private _showPastedToastWithUndo() {
    showToast(this, {
      message: this.hass.localize(
        "ui.panel.config.developer-tools.tabs.blueprints.paste_toast_message"
      ),
      duration: 4000,
      action: {
        ...
        <div class="content-wrapper">
          ...
          <div class="header">
            <h2 id="variables-heading" class="name">
              ${this.hass.localize(
                "ui.panel.config.developer-tools.tabs.blueprints.editor.inputs.header"
              )}
            </h2>
            <a
            ...
            >
              <ha-icon-button
                ...
                .label=${this.hass.localize(
                  "ui.panel.config.developer-tools.tabs.blueprints.editor.inputs.learn_more"
                )}
              ></ha-icon-button>
            </a>
          </div>
          ${!Object.entries(this.blueprint?.blueprint?.input || {})?.length
            ? html`<p class="section-description">
                ${this.hass.localize(
                  "ui.panel.config.developer-tools.tabs.blueprints.editor.inputs.section_description"
                )}
              </p>`
            : nothing}
          ...
          <div class="actions">
            ...
            >
              ${this.hass.localize(
                "ui.panel.config.developer-tools.tabs.blueprints.editor.actions.reset"
              )}
            </ha-button>
          </div>
        </div>
