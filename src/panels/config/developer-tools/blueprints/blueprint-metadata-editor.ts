...
class BlueprintMetadataEditor extends LitElement {
  ...
  private _computeLabel(step: SchemaUnion<typeof SCHEMA>) {
    return this.hass.localize(
      `ui.panel.config.developer-tools.tabs.blueprints.editor.${step.name}.label`
    );
  }
    ...
    return html`
      <div class="container">
        <div class="header">
          <h2 class="name">
            ${this.hass.localize(
              "ui.panel.config.developer-tools.tabs.blueprints.editor.metadata"
            )}
          </h2>
        </div>
        <div class="content">
    ...
    return [
      ...
      css`
        ...
        .header {
          display: flex;
          align-items: center;
          margin-top: calc(var(--ha-space-6) * -1);
        }
