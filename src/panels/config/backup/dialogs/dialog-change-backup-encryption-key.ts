import { mdiClose, mdiContentCopy, mdiDownload, mdiKey } from "@mdi/js";
...
class DialogChangeBackupEncryptionKey extends LitElement implements HassDialog {
  ...
  public showDialog(params: ChangeBackupEncryptionKeyDialogParams): void {
    this._params = params;
    this._step = this._params?.currentKey ? STEPS[0] : STEPS[1];
    this._opened = true;
    this._suggestedEncryptionKey = window.crypto
    ...
    return html`
      <ha-md-dialog open @closed=${this.closeDialog}>
        <ha-dialog-header slot="headline">
          ${this._params?.currentKey && this._step === "new"
            ? html`
                <ha-icon-button-prev
                  slot="navigationIcon"
        ...
        return html`
          ...
          <div class="row">
            <ha-password-field
              ...
              .value=${this._newEncryptionKey!}
            ></ha-password-field>
            <ha-button @click=${this._copyNewEncryptionKey}>
  ...
  private async _copyNewEncryptionKey() {
  ...
  private async _submit() {
    ...
    this._params!.submit?.(this._newEncryptionKey);
    this._nextStep();
  }
