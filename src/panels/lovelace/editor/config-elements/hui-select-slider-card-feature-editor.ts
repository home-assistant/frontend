...
type SelectSliderCardFeatureData = SelectSliderCardFeatureConfig & {
  customize_options: boolean;
};

@customElement("hui-select-slider-card-feature-editor")
export class HuiSelectSliderCardFeatureEditor
  extends LitElement
  implements LovelaceCardFeatureEditor
{
  ...
  @state() private _config?: SelectSliderCardFeatureConfig;

  public setConfig(config: SelectSliderCardFeatureConfig): void {
    this._config = config;
  }
  ...
  protected render() {
    ...
    const data: SelectSliderCardFeatureData = {
      ...this._config,
  ...
  private _valueChanged(ev: CustomEvent): void {
    const { customize_options, ...config } = ev.detail
      .value as SelectSliderCardFeatureData;
    ...
    switch (schema.name) {
      ...
      case "customize_options":
        return this.hass!.localize(
          `ui.panel.lovelace.editor.features.types.select-slider.${schema.name}`
        );
      default:
        return "";
    }
...
declare global {
  interface HTMLElementTagNameMap {
    "hui-select-slider-card-feature-editor": HuiSelectSliderCardFeatureEditor;
  }
}
