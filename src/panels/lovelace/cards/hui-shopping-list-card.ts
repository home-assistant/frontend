...
class HuiShoppingListCard extends hassLocalizeLitMixin(LitElement)
  ...
  private async _fetchData(): Promise<void> {
    if (this.hass) {
      ...
      const items = await fetchItems(this.hass, LIST_SYSTEM_INBOX);
  ...
  private _completeItem(ev): void {
    updateItem(this.hass!, LIST_SYSTEM_INBOX, ev.target.itemId, {
  ...
  private _clearItems(): void {
    if (this.hass) {
      clearItems(this.hass, LIST_SYSTEM_INBOX).catch(() => this._fetchData());
    }
  }
  ...
  private _addItem(ev): void {
    ...
    if (newItem.value!.length > 0) {
      addItem(this.hass!, LIST_SYSTEM_INBOX, newItem.value!).catch(() =>
        this._fetchData()
      );
    }
