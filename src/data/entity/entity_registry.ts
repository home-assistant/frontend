...
const entityDisplayEntriesEqual = (
...
/**
 * The entity registry display is fetched and rebuilt in full on every
 * `entity_registry_updated` event, producing brand-new entry objects for every
 * entity. Without this, a change to a single entity (or an event that doesn't
 * touch the display payload at all) gives every entity a new object reference,
 * so all entity-bound consumers needlessly re-render.
 *
 * This reuses the previous entry object whenever an entity is unchanged, and
 * returns the previous record untouched when nothing changed at all, so only
 * genuinely-changed entities (and their consumers) update.
 */
export const preserveUnchangedEntityRegistryDisplay = (
  previous: Record<string, EntityRegistryDisplayEntry> | undefined,
  next: Record<string, EntityRegistryDisplayEntry>
  ...
  for (const entityId of Object.keys(next)) {
    const previousEntry = previous[entityId];
    if (
      previousEntry &&
      entityDisplayEntriesEqual(previousEntry, next[entityId])
    ) {
      next[entityId] = previousEntry;
    } else {
      changed = true;
    }
  }
